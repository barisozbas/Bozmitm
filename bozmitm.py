...
try:
    print("Please read README file.")
    
    update_quest = input("Install/update needed modules? [Y/N]")
    ...
    while(True):
        pw = input("Enter a valid password for AP (8 or more character needed) or press enter for no password (recommended): ")
        if(pw != ""):
    ...
    https_quest = input("[mitmproxy] Capture HTTPS traffic with mitmproxy? (certificate needed, download it from mitm.it) [Y/N] ")
    ...
    os.system("sudo iptables --delete-chain")
    os.system("sudo iptables --table nat --delete-chain")
    print("[bozmitm] Reboot your pi to reset all internet configurations. ")
